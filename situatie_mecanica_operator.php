	...
	<div id="control_panel" class="hide_prt">
		<table width="100%" style="border-bottom:1px solid #dfdfdf;">
			<tr>
				...
				<td width="60%"><input type="button" '.((($ultima_data == null) || ($data1.'' != $today.''))?("disabled"):("")).' value="Vezi ultima situatie completata" onclick="document.location='."'".'situatie_mecanica_operator.php?last=true'."'".'" />&nbsp;&nbsp;
				...
				<td style="text-align:right">
					<input type="button" class="mod bold" '.((($ultima_data == null) || ($data1.'' != $today.''))?("disabled"):("")).' title="Salveaza toate modificarile" id="salveaza_modificari" value="Salvează modificările !" onclick="beforeSubmit()" />
				</td>
			</tr>
			<tr>
				<td width="50%">
		...
		<table width="100%" >
			<tr>
				<td style="">
					...
					<input type="button"   value="Realizeaza o dispozitie noua" onclick="document.location='."'".'dispozitie_operator_noua.php?from='.$data1->getAnul().'-'.$data1->getLuna()."'".'" />
				</td>
	...
	echo '	<script>
		...
		$dompdf->render();
		$dompdf->stream("Situație Mec. pentru ".$data1." pentru ".$firma->getDenumire().".pdf");
	}	
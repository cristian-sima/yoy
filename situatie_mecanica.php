	...
	if(!isset($_GET['type']) || (isset($_GET['type']) && $_GET['type'] != "PDF")){
		...
		<div id="control_panel" class="disp">
			...
			<option value="afisarea" '.(((isset($_GET['type']))&&($_GET['type']=="afisarea"))?"selected='selected'":"").'>Afisarea</option>
			</select>&nbsp;&nbsp;<input type="button" value="Proceseaza !" onclick="seeData();" id="viz_" />
			';	
		echo '</td><td style="text-align:right">';
		echo '<a style="display:none"></a><a href="details.php?idFirma='.$firma->getID().'"><input type="button" value="Interfata firma" /></a>&nbsp;
				<a href="aparate.php?id='.$firma->getID().'"><input type="button" title="Lista cu aparate din firma" value="Aparate" /></a>';	
		...
		echo '<input title="'.$ultima_data.'"'.(((isset($_GET['month']))) || ($ultima_data == null)?'disabled="disabled"':'').' type="button" id="a" value="Completarea precedenta" onclick="document.location='."'".'situatie_mecanica.php?id_firma='.$firma->getID().'&from='.$ultima_data.'&to='.$ultima_data."'".'"/>';
		echo '<input title="'.$today.'"'.(($data1 == $today)?'disabled="disabled"':'').' type="button" id="a" value="Ziua curenta" onclick="document.location='."'".'situatie_mecanica.php?id_firma='.$firma->getID().'&from='.$today.'&to='.$today."'".'"/>';
		...
		echo '</td><td style="text-align:right">&nbsp;&nbsp;&nbsp;';
		echo '<a href="situatie_mecanica.php?id_firma='.$firma->getID().'&from='.$data1.'&to='.$data2.'&type=PDF"><input type="button" value="Descarca situatie" /></a>&nbsp;&nbsp;&nbsp;';		
		echo '<input type="button" value="Printează situatie"  onclick="window.print()" />&nbsp;&nbsp;&nbsp;';		
